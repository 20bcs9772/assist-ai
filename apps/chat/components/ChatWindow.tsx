...
import { Send, MoreVertical, Sparkles, Sun, Moon, Trash2, ChevronDown, Infinity, Info } from 'lucide-react';
...
const AGENTS: Record<Exclude<AgentType, 'auto'>, { label: string, info: string }> = {
  support: { label: 'Support', info: 'Specialized in technical troubleshooting and documentation.' },
  order: { label: 'Logistics', info: 'Handles tracking, shipping, and order management queries.' },
  billing: { label: 'Billing', info: 'Resolves questions about invoices, payments, and plans.' }
};
...
const getAvatarColor = (name: string) => {
  const colors = ['bg-blue-500', 'bg-purple-500', 'bg-pink-500', 'bg-indigo-500', 'bg-teal-500', 'bg-orange-500', 'bg-emerald-500'];
  let hash = 0;
...
export const ChatWindow: React.FC<ChatWindowProps> = ({ 
  chat, onSendMessage, onDeleteChat, streamingStatus, isThinking = false, theme, onToggleTheme, isLoading = false
}) => {
  const [input, setInput] = useState('');
  const [showOptions, setShowOptions] = useState(false);
  const [showAgentDropdown, setShowAgentDropdown] = useState(false);
  const [hoveredAgent, setHoveredAgent] = useState<keyof typeof AGENTS | null>(null);
    ...
    return (
      <div className="flex-1 flex flex-col h-full bg-transparent">
        <header className="h-20 flex items-center justify-end px-10 border-b border-slate-200 dark:border-white/5">
           <button onClick={onToggleTheme} className="p-2.5 rounded-2xl bg-slate-100 dark:bg-slate-900 border border-slate-200 dark:border-white/5 text-slate-500 dark:text-slate-400 hover:text-orange-500 dark:hover:text-white transition-all">
        ...
        <div className="flex-1 flex flex-col items-center justify-center text-slate-300 dark:text-slate-700">
          <div className="p-10 rounded-[3rem] border border-slate-200 dark:border-white/5 bg-white/40 dark:bg-slate-900/20 text-center">
            <Sparkles size={48} className="mx-auto mb-6 opacity-20 text-orange-500" />
            <h2 className="text-xl font-medium text-slate-500 dark:text-slate-400 mb-2">Ready to assist</h2>
            <p className="text-sm opacity-50 max-w-[200px]">Select a conversation to begin your AI-powered session.</p>
          </div>
        </div>
      </div>
    );
    ...
    <div className="flex-1 flex flex-col h-full overflow-hidden relative">
      ...
      <header className="h-20 flex items-center justify-between px-10 border-b border-slate-200 dark:border-white/5 bg-transparent shrink-0">
        <div className="flex items-center gap-4">
          <div className="relative">
            <div className={`w-10 h-10 rounded-2xl flex items-center justify-center text-white text-xs font-bold ring-1 ring-slate-200 dark:ring-white/10 ${getAvatarColor(chat.name)}`}>
          ...
          <div>
            <h2 className="text-base font-semibold text-slate-900 dark:text-white tracking-tight">{chat.name}</h2>
            <div className="flex items-center gap-1.5">
              <span className="w-1 h-1 bg-green-500 rounded-full animate-pulse"></span>
              <span className="text-[10px] font-bold uppercase tracking-widest text-slate-400 dark:text-slate-500">Live Connection</span>
            </div>
          </div>
        </div>
        <div className="flex items-center gap-4 relative" ref={optionsRef}>
          <button onClick={onToggleTheme} className="p-2.5 rounded-2xl bg-slate-100 dark:bg-slate-900 border border-slate-200 dark:border-white/5 text-slate-500 dark:text-slate-400 hover:text-orange-500 dark:hover:text-white transition-all shadow-sm">
            {theme === 'light' ? <Moon size={20} /> : <Sun size={20} />}
          </button>
          <button onClick={() => setShowOptions(!showOptions)} className="p-2 text-slate-400 dark:text-slate-500 hover:text-slate-900 dark:hover:text-white transition-colors">
          ...
          {showOptions && (
            <div className="absolute top-full right-0 mt-2 w-52 bg-white dark:bg-slate-900 border border-slate-200 dark:border-white/10 rounded-2xl shadow-2xl z-[60] overflow-hidden animate-in fade-in slide-in-from-top-2">
              <button 
                onClick={() => { onDeleteChat(); setShowOptions(false); }}
      ...
      <div ref={scrollRef} className="flex-1 overflow-y-auto px-10 py-8 space-y-8 scroll-smooth">
        {chat.messages.map((msg) => (
          <div key={msg.id} className={`flex ${msg.role === 'USER' ? 'justify-end' : 'justify-start'}`}>
            <div className={`max-w-[70%] ${msg.role === 'USER' ? 'items-end' : 'items-start'} group`}>
              <div className={`px-5 py-4 rounded-[1.75rem] text-[0.9375rem] leading-relaxed shadow-lg ${msg.role === 'USER' ? 'bg-orange-500 text-white rounded-tr-none shadow-orange-500/20' : 'bg-white dark:bg-slate-800 text-slate-800 dark:text-slate-100 rounded-tl-none border border-slate-200 dark:border-white/5'}`}>
                <MessageContent content={msg.content} isUser={msg.role === 'USER'} />
                {msg.isStreaming && <span className={`inline-block ml-1 w-1.5 h-4 cursor-blink align-middle ${msg.role === 'USER' ? 'bg-white/50' : 'bg-orange-500/50'}`}></span>}
              </div>
              <div className={`flex items-center gap-2 mt-2 px-1 ${msg.role === 'USER' ? 'justify-end' : 'justify-start'}`}>
                <span className="text-[9px] font-bold text-slate-400 dark:text-slate-600 uppercase tracking-widest">{new Date(msg.timestamp).toLocaleTimeString([], { hour: '2-digit', minute: '2-digit' })}</span>
              </div>
            </div>
          </div>
        ))}
      ...
      <div className="px-6 pb-6 pt-2 shrink-0">
        <form onSubmit={handleSubmit} className="max-w-4xl mx-auto relative">
          ...
          {hoveredAgent && tooltipPosition && (
             <div 
               ...
               onMouseLeave={(e) => {
                 ...
                 if (!relatedTarget || (!relatedTarget.closest('[data-agent-info]') && !relatedTarget.closest('[data-agent-tooltip]'))) {
                   setHoveredAgent(null);
                   setTooltipPosition(null);
                 }
               }}
             >
               <div className="flex items-center gap-2 mb-2">
                 ...
                 <span className="text-[10px] font-bold uppercase tracking-wider text-orange-500">{AGENTS[hoveredAgent].label} Expert</span>
               </div>
               <p className="text-[11px] text-white dark:text-slate-200 leading-relaxed font-semibold">{AGENTS[hoveredAgent].info}</p>
            ...
            <div className="flex items-center justify-between mt-4">
              <div className="flex items-center gap-2 relative" ref={dropdownRef}>
                ...
                >
                  ...
                  <span>Agent: <span className="text-slate-900 dark:text-white">Auto</span></span>
                  <ChevronDown size={14} className={`text-slate-400 transition-transform duration-300 ${showAgentDropdown ? 'rotate-180' : ''}`} />
                </button>
                ...
                {showAgentDropdown && (
                  <div className="absolute bottom-full left-0 mb-3 w-56 bg-white dark:bg-slate-900 border border-slate-200 dark:border-white/10 rounded-2xl shadow-2xl z-[70] overflow-hidden p-2 animate-in fade-in slide-in-from-bottom-2">
                    <div className="px-3 py-2 mb-2 bg-orange-500/10 dark:bg-orange-500/5 rounded-xl">
                      <p className="text-[10px] text-orange-600 dark:text-orange-400 font-bold leading-tight">Agents are auto-selected based on your input to provide the best response.</p>
                    </div>
                    {(Object.keys(AGENTS) as (keyof typeof AGENTS)[]).map((type) => (
                      <div
                        key={type}
                      ...
                      >
                        ...
                        <div 
                          ...
                          onMouseLeave={(e) => {
                            const relatedTarget = e.relatedTarget as HTMLElement;
                            if (!relatedTarget || (!relatedTarget.closest('[data-agent-tooltip]') && !relatedTarget.closest('[data-agent-info]'))) {
                              setHoveredAgent(null);
                              setTooltipPosition(null);
                            }
                          }}
                        >
                          <Info size={14} className={`transition-opacity ${hoveredAgent === type ? 'opacity-100 text-orange-500' : 'opacity-40 text-slate-400'}`} />
                        </div>
                      </div>
                    ))}
                  </div>
          ...
          <div className="mt-4 flex justify-center items-center gap-4 text-[9px] font-bold text-slate-400 dark:text-slate-700 uppercase tracking-[0.2em]">
            <span>Powered by Assist-AI</span>
