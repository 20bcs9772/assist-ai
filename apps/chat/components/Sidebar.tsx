        ...
        <div className="flex-1 overflow-y-auto px-4 space-y-2 pb-8">
          {conversations.length === 0 ? (
            <div className="flex flex-col items-center justify-center h-full text-center px-4">
              <div className="text-slate-400 dark:text-slate-600 text-sm mb-2">No conversations yet</div>
              <div className="text-slate-300 dark:text-slate-700 text-xs">Create a new chat to get started</div>
            </div>
          ) : (
            conversations.map(chat => (
              ...
              >
                <div className="relative flex-shrink-0">
                  <div className={`w-11 h-11 rounded-2xl flex items-center justify-center text-white text-sm font-bold ring-2 ring-slate-200 dark:ring-white/5 ${getAvatarColor(chat.name)}`}>
                ...
                <div className="flex-1 text-left overflow-hidden">
                  <div className={`text-sm font-medium truncate ${activeChatId === chat.id ? 'text-orange-600 dark:text-orange-500' : 'text-slate-900 dark:text-slate-100'}`}>
                    {chat.name}
                  </div>
        ...
        <div className="fixed inset-0 z-[100] flex items-center justify-center p-4 bg-slate-900/40 dark:bg-slate-950/60 backdrop-blur-md animate-in fade-in duration-200">
          <div className="w-full max-w-md bg-white dark:bg-slate-900 rounded-[2.5rem] p-10 shadow-2xl border border-slate-200 dark:border-white/10 transform transition-all animate-in zoom-in-95 duration-200">
            <div className="flex justify-between items-center mb-8">
              <h3 className="text-2xl font-bold tracking-tight text-slate-900 dark:text-white">Create Conversation</h3>
              <button onClick={() => setShowModal(false)} className="p-2 text-slate-400 hover:text-slate-600 dark:hover:text-white transition-colors">
                <X size={24} />
              </button>
            </div>
            <form onSubmit={handleCreateChat}>
              <label className="block text-[10px] font-bold uppercase tracking-[0.2em] text-slate-500 mb-3 ml-1">Contact Name</label>
              <input 
                autoFocus
                type="text"
